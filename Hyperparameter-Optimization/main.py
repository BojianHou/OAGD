...
def main(seed, args):
    ...
    if dataset == 'cifar10' or dataset == 'cifar100' or dataset == 'tadpole' \
            or dataset == 'mnist' or dataset == 'adult':
        ...
        warm_up_with_cosine_lr_low = lambda epoch: \
            (epoch + 1) / args["low_lr_warmup"] if epoch < args["low_lr_warmup"] \
                ...
                (epoch - args["low_lr_warmup"]) / (args["epoch"] - args["low_lr_warmup"]) * math.pi) + 1)
        up_start_epoch = args["up_configs"]["start_epoch"]
        ...
        train_optimizer = optim.SGD(
            ...
            lr=args["inner_lr"], momentum=0.9, weight_decay=5e-4)
        val_optimizer = optim.SGD(params=[{'params': dy}, {'params': ly}],
                                  lr=args["outer_lr"], momentum=0.9, weight_decay=5e-4)
        train_lr_scheduler = optim.lr_scheduler.LambdaLR(
...
if __name__ ==  '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='tadpole')  # adult, mnist, tadpole, cifar10, cifar100
    ...
    config = f'configs/{args.dataset}/dyly_no_init/1.yaml'
    ...
    up_lr = args['outer_lr']
    # new means I changed the structure of the online autobalance. Previously I
    if method == 'OAGD':
        with open(f'results/{dataset}/results_{method}_win_size_{win_size}'
                  f'_batch_size_{batch_size}_low_lr_{low_lr}_up_lr_{up_lr}_gamma_{gamma}_late_lr_decay.pkl', 'wb') as f:
            pkl.dump(results, f)
    else:
        with open(f'results/{dataset}/results_{method}_batch_size_{batch_size}_low_lr_{low_lr}_up_lr_{up_lr}_late_lr_decay.pkl', 'wb') as f:
            pkl.dump(results, f)
